...
export const betweenZeroAndOne = (val: number): number =>
	assert(Math.abs(val) <= 1, 'Value must be a number between 0 and 1.') && val;

export const normalizeTrack = (val: number | TrackShorthand | `${TrackShorthand}`): number => {
	if (isNumber(val)) {
		return val;
	}
...
export const toUnitTuple = (val: number | string): [value: number, unit: string] => {
	if (isNumber(val)) {
		return [val, 'px'];
	}
	try {
